...
class StopwatchWithResults extends Stopwatch {
  ...
  private results = [] as Array<string>;
  ...
  prepareElements(element: HTMLDivElement): void {
    /*
        * stopwatch__results
        * stopwatch__button stopwatch__add-to-list-btn
        * stopwatch__button stopwatch__reset-list-btn
    * */

    this.dom.resultsList = <HTMLDivElement>element.querySelector('.stopwatch__results');
    this.dom.addToListBtn = <HTMLButtonElement>element.querySelector('.stopwatch__start-add-to-list');
    this.dom.resetListBtn = <HTMLButtonElement>element.querySelector('.stopwatch__start-reset-list');
  }
  ...
  renderList(): void {
    ...
    */
    ...
    if (this.results) {
      this.results.forEach(result => this.dom.resultsList.appendChild(document.createTextNode(`<li>${result}</li>`)));
    }
  ...
  addToList(): void {
    ...
    */
    ...
    this.results.push(this.dom.currentTime.innerText);
    this.renderList()
  }
